...
function ServiceSelection() {
  ...
  const handleSourceChange = (event) => {
    ...
    axios
      .get(`http://localhost:3000/destinations?source=${event.target.value}`)
      .then((response) => setDestinations(response.data))
      .catch((error) => console.error("Error fetching destinations:", error));
  };
  ...
  const handleDestinationChange = (event) => {
    ...
    axios
      .get(
        `http://localhost:3000/departureTimes?source=${selectedSource}&destination=${event.target.value}`
      )
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post("http://localhost:3000/storedata", data)
      .then((response) => {
